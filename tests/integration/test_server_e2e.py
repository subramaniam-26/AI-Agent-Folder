...
def is_model_availability_error(events: list[dict[str, Any]]) -> bool:
    """Detect transient or quota-related model errors in ADK SSE events."""
    ...
    if any(marker in rendered_events for marker in known_markers):
        return True

    return any(
        event.get("error_code") or event.get("error_message") for event in events
    )
...
def test_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    if not has_text_content and is_model_availability_error(events):
        pytest.skip("Gemini model is temporarily unavailable or quota-limited.")
